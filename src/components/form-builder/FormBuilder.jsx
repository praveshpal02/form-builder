...
export default function FormBuilder({
  ...
  const isDirty = useMemo(() => {
    if (!lastSavedString) return mode === "create";
    return JSON.stringify(formSchema) !== lastSavedString;
  }, [formSchema, mode, lastSavedString]);
  ...
  const handlePublish = useCallback(async () => {
    ...
    if (!validation.valid) {
      setPublishError("Please fix form errors before publishing.");
      return;
    }
  ...
  return (
    <div className="flex flex-col min-h-[calc(100vh-4rem)] bg-background">
      {/* Top Header Bar */}
      <header className="sticky top-16 z-30 flex items-center justify-between border-b border-border bg-card/90 backdrop-blur-sm px-4 py-3 sm:px-8">
        ...
        {/* Center Tabs */}
        <nav className="hidden sm:flex items-center gap-1 absolute left-1/2 -translate-x-1/2" aria-label="Form editor tabs">
          {[
            { id: "customize", label: "Customize", icon: "M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" },
        ...
        <div className="flex items-center gap-3">
          ...
          {/* Mobile Tabs */}
          <nav className="sm:hidden flex items-center gap-1" aria-label="Form editor tabs">
            {[
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-xl border border-border bg-background px-3 py-1.5 text-xs font-medium text-foreground hover:bg-muted transition-colors shadow-2xs"
          >
      ...
      {activeTab === "customize" ? (
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-8 flex justify-center bg-background">
          <div className="w-full max-w-xl space-y-8 self-start pt-4">
            {/* Status Card */}
                  ...
                  {publicUrl && (
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-2">
                        <div className="flex-1 min-w-0 rounded-xl border border-border bg-background px-4 py-2.5 text-sm text-foreground font-mono truncate">
                          {publicUrl}
                        </div>
                        <button
                          type="button"
                          onClick={handleCopyLink}
                          className={`inline-flex items-center gap-1.5 rounded-xl border px-3 py-2.5 text-xs font-medium transition-colors shrink-0 ${
                            copied
                  ...
                  <div className="flex items-center gap-3">
                    {publicUrl && (
                      <a
                        ...
                        className="inline-flex items-center gap-1.5 rounded-xl border border-border bg-background px-4 py-2.5 text-xs font-medium text-foreground hover:bg-muted transition-colors"
                      >
                        <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 rounded-xl border border-border bg-background px-4 py-2.5 text-xs font-medium text-foreground hover:bg-muted transition-colors disabled:opacity-50"
                    >
                      <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
